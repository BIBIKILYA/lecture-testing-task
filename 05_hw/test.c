...
// Макрос для вывода результатов
#define PASS(msg) printf("%s - PASSED\n", msg)

// Базовые тесты (без изменений)
...
// Новые тесты: Увеличение покрытия
void testInitStackMultipleNull() {
    for (int i = 0; i < 5; ++i) {
        initStack(NULL); // Покрываем строку 20
    }
...
void testIsEmptyMultipleNull() {
    for (int i = 0; i < 5; ++i) {
        assert(isEmpty(NULL) == true); // Покрываем строку 29
    }
...
void testPushMultipleNull() {
    for (int i = 0; i < 5; ++i) {
        push(NULL, i); // Покрываем строку 38
    }
...
void testCombinedNullOperations() {
    ...
    push(NULL, 10);
    pop(NULL);
    destroyStack(NULL); // Тесты вызовов с NULL для всех функций
